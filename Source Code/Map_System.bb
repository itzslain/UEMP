...
Type Rooms
	...
	Field ObjectUsed%[MaxRoomObjects]
	Field Levers%[11]
		...
		if hide then
			if multiplayer_IsAHost() then
				For p.players = Each players
					if p\playerRoomID <> 0 Then
						if r\ID = p\PlayerRoomID Then
							hide = False
							Exit
						EndIf
						If hide Then
							If IsRoomAdjacent(Room[p\PlayerRoomID],r) Then hide = False : Exit
						EndIf
						If hide Then
							For i=0 To 3
								If (IsRoomAdjacent(Room[p\PlayerRoomID]\Adjacent[i],r)) Then hide=False : Exit
							Next
						EndIf
						if hide = False Then Exit
					EndIf
				Next
			endif
		endif
...
Function UpdateElevators#(State#, door1.Doors, door2.Doors, room1, room2, event.Events, ignorerotation% = True)
	...
	If door1\open Then
		...
		If Abs(EntityX(GetPreviousEventCollider())-EntityX(room1,True))<280.0*RoomScale+(0.015*fs\FPSfactor[0]) Then
			If Abs(EntityZ(GetPreviousEventCollider())-EntityZ(room1,True))<280.0*RoomScale+(0.015*fs\FPSfactor[0]) Then	
				If Abs(EntityY(GetPreviousEventCollider())-EntityY(room1,True))<280.0*RoomScale+(0.015*fs\FPSfactor[0]) Then	
					door1\locked = False
					door1\IsElevatorDoor = 1
				EndIf
			EndIf
	...
	If door2\open Then
		...
		If Abs(EntityX(GetPreviousEventCollider())-EntityX(room2,True))<280.0*RoomScale+(0.015*fs\FPSfactor[0]) Then
			If Abs(EntityZ(GetPreviousEventCollider())-EntityZ(room2,True))<280.0*RoomScale+(0.015*fs\FPSfactor[0]) Then	
				If Abs(EntityY(GetPreviousEventCollider())-EntityY(room2,True))<280.0*RoomScale+(0.015*fs\FPSfactor[0]) Then
					door2\locked = False
					door2\IsElevatorDoor = 1
				EndIf
			EndIf
		...
		If door1\openstate = 0 And door2\openstate = 0 Then
				...
				If State < -500 Then
					...
					If inside Then
						If (Not ignorerotation) Then
							dist# = Distance(EntityX(GetPreviousEventCollider(),True),EntityZ(GetPreviousEventCollider(),True),EntityX(room1,True),EntityZ(room1,True))
							dir# = point_direction(EntityX(GetPreviousEventCollider(),True),EntityZ(GetPreviousEventCollider(),True),EntityX(room1,True),EntityZ(room1,True))
							...
							RotateEntity GetPreviousEventCollider(),EntityPitch(GetPreviousEventCollider(),True),EntityYaw(room2,True)+angleDist(EntityYaw(GetPreviousEventCollider(),True),EntityYaw(room1,True)),EntityRoll(GetPreviousEventCollider(),True),True
						Else
							x# = Max(Min((EntityX(GetPreviousEventCollider())-EntityX(room1,True)),280*RoomScale-0.22),-280*RoomScale+0.22)
							z# = Max(Min((EntityZ(GetPreviousEventCollider())-EntityZ(room1,True)),280*RoomScale-0.22),-280*RoomScale+0.22)
						EndIf
						
						TeleportEntity(GetPreviousEventCollider(), EntityX(room2,True)+x,(0.1*fs\FPSfactor[0])+EntityY(room2,True)+(EntityY(GetPreviousEventCollider())-EntityY(room1,True)),EntityZ(room2,True)+z,0.3,True)
						UpdateDoorsTimer = 0
						DropSpeed = 0
					...
					UseDoor(door2,False,Not inside, False)
					door1\open = False
			...
			Else
				...
				If State > 500 Then 
					...
					If inside Then	
						If (Not ignorerotation) Then
							dist# = Distance(EntityX(GetPreviousEventCollider(),True),EntityZ(GetPreviousEventCollider(),True),EntityX(room2,True),EntityZ(room2,True))
							dir# = point_direction(EntityX(GetPreviousEventCollider(),True),EntityZ(GetPreviousEventCollider(),True),EntityX(room2,True),EntityZ(room2,True))
							dir=dir+EntityYaw(room1,True)-EntityYaw(room2,True)
							x# = Max(Min(Cos(dir)*dist,280*RoomScale-0.22),-280*RoomScale+0.22)
							z# = Max(Min(Sin(dir)*dist,280*RoomScale-0.22),-280*RoomScale+0.22)
							RotateEntity GetPreviousEventCollider(),EntityPitch(GetPreviousEventCollider(),True),EntityYaw(room2,True)+angleDist(EntityYaw(GetPreviousEventCollider(),True),EntityYaw(room1,True)),EntityRoll(GetPreviousEventCollider(),True),True
						Else
							x# = Max(Min((EntityX(GetPreviousEventCollider())-EntityX(room2,True)),280*RoomScale-0.22),-280*RoomScale+0.22)
							z# = Max(Min((EntityZ(GetPreviousEventCollider())-EntityZ(room2,True)),280*RoomScale-0.22),-280*RoomScale+0.22)
						EndIf
						
						TeleportEntity(GetPreviousEventCollider(), EntityX(room1,True)+x,(0.1*fs\FPSfactor[0])+EntityY(room1,True)+(EntityY(GetPreviousEventCollider())-EntityY(room2,True)),EntityZ(room1,True)+z,0.3,True)
						UpdateDoorsTimer = 0
						DropSpeed = 0
					...
					UseDoor(door1,False,Not inside, False)
					door2\open = False
