...
Global timedport$ = "51110", timedip$ = "localhost"
...
Function UpdateMainMenu()
	...
	if multiplayerErrorTime > 0 Then
		Color 255, 0, 0
		AAText GraphicWidth-AAStringWidth(multiplayerError)-10, GraphicHeight-30, multiplayerError
		multiplayerErrorTime = multiplayerErrorTime - fs\FPSfactor[0]
		Color 255,255,255
	EndIf
	...
	If ms\MenuBlinkTimer[1] < ms\MenuBlinkDuration[1] Then
		...
		If ms\MenuBlinkTimer[1] < 0 Then
			...
			Select Rand(0, 23)
				...
				Case 15
					ms\MenuStr = "Stone does not care"
				Case 16
					ms\MenuStr = "9341"
				Case 17
	...
	Else
		...
		Select ms\MainMenuTab
			Case 1 ; New game
				...
				Color(255, 255, 255)
				AASetFont fo\Font[1]
				AAText(GraphicWidth/2, y + height / 2, "MULTIPLAYER", True, True)
				...
				AAText (x + 150 * MenuScale, y - 5 * MenuScale, "IP")
				timedip = InputBox(x + 70 * MenuScale, y + 15 * MenuScale, 200 * MenuScale, 30 * MenuScale, timedip, 234)
				
				AAText (x + 370 * MenuScale, y - 5 * MenuScale, "Port")
				timedport = InputBox(x + 300 * MenuScale, y + 15 * MenuScale, 200 * MenuScale, 30 * MenuScale, timedport, 657)
				...
				If DrawButton(x + 450 * MenuScale, y + height + 20 * MenuScale, 160 * MenuScale, 70 * MenuScale, "HOST", False) Then
					if multiplayer_starthost(timedport) then
				...
				If DrawButton(x + 260 * MenuScale, y + height + 20 * MenuScale, 160 * MenuScale, 70 * MenuScale, "CONNECT", False) Then
					if multiplayer_connect(timedip, timedport) <> 0 then
						If CurrSave = "" Then CurrSave = "untitled"
				...
				if MouseOn(x + 450 * MenuScale, y + height + 20 * MenuScale, 160 * MenuScale, 70 * MenuScale) then 
					DrawDynamicBoxText(ScaledMouseX(), ScaledMouseY()-30*MenuScale, 250*MenuScale, 30*MenuScale, "Requires only Port value")
				EndIf
				
				if MouseOn(x + 260 * MenuScale, y + height + 20 * MenuScale, 160 * MenuScale, 70 * MenuScale) then 
					DrawDynamicBoxText(ScaledMouseX(), ScaledMouseY()-30*MenuScale, 250*MenuScale, 30*MenuScale, "Requires IP and Port value")
				EndIf
			...
			Case 3,5,6,7 ;options
				...
				ElseIf ms\MainMenuTab = 6 ;Controls
					...
					Color 0, 200, 0
					AAText(x + 20 * MenuScale, y + 120 * MenuScale, "Voice")
					Color 255,255,255
					
					InputBox(x + 160 * MenuScale, y + 120 * MenuScale,100*MenuScale,20*MenuScale,KeyName(Min(KEY_VOICE,210)),14)
...
Function FPSMainMenu()
	...
	If ShowFPS Then AASetFont fo\ConsoleFont : AAText 20, GraphicHeight-70, "FPS: " + fs\FPS : AASetFont fo\Font[0]
...
Function UpdateLauncher()
	...
	AppTitle "SCP - CB: Ultimate Edition Launcher"
...
Function DrawDynamicBoxText(x%, y%, width%, height%, txt$)
	DrawFrame (x, y, width, height)
