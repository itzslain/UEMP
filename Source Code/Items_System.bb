...
Type Items
	...
	Field picker%, pickerplayer.players
	field waitfactor
	Field x#, y#, z#
End Type 
...
Function CreateItem.Items(name$, tempname$, x#, y#, z#, r%=0,g%=0,b%=0,a#=1.0,invSlots%=0)
	...
	if instr(name, "Night Vision Goggles") Or instr(name, "radio") then i\state = 1000000000
	
	i\invSlots=invSlots
	
	i\ID=findfreeitemid(i)
	EntityType i\collider,HIT_ITEM
	
	if ShouldSendItem Then multiplayer_CreateNewItem(i)
	Return i
End Function

Function CreateItemByTemplate.Items(it.itemtemplates, x#, y#, z#, r%=0,g%=0,b%=0,a#=1.0,invSlots%=0)
	Local i.Items = New Items
	;Local o.Objects = First Objects
	...
	name$ = Lower(it\name)
	tempname$ = Lower (it\tempname)
	
	i\WontColl = False
	
	If i\itemtemplate = Null Then Return
...
Function RemoveItem(i.Items, send = true)
	Local n
...
Function UpdateItems()
	...
	For i.Items = Each Items
		...
		If (Not i\Picked) Then
			...
			If i\dist < HideDist or nearestplayer = true Then
				ShowEntity i\collider
		...
		If i\picker = multiplayer\MyID Then
			foundinv = False
		...
		Else
			For a = 0 To MaxItemAmount-1
				if Inventory(a) <> Null Then
					If Inventory(a) = i Then Inventory(a) = Null : Exit
					For b = 0 To Inventory(a)\invSlots-1
						if Inventory(a)\SecondInv[b] = i Then
...
Function PickItem(item.Items, shouldplaysound%=true)
	Local n% = 0
	...
	If (Not fullINV) Then
		For n% = 0 To MaxItemAmount - 1
			If Inventory(n) = Null Then
				...
				If item\itemtemplate\sound <> 66 and shouldplaysound = true Then 
					PlaySound_Strict(PickSFX(item\itemtemplate\sound))
					multiplayer_sendSound(PickSFX(item\itemtemplate\sound), 10, 0.8)
				EndIf
				item\Picked = True
				item\picker = myplayer\ID
				item\Dropped = -1
...
Function Update294()
	...
	If VomitTimer > 0 Then
		...
		If Rand(50) = 50 And (MilliSecs2() Mod 4000) < 200 Then 
		...
		If VomitTimer < 10 And Rnd(0, 500 * VomitTimer) < 2 Then
			If (Not ChannelPlaying(VomitCHN)) And (Not Regurgitate) Then
				...
				multiplayer_sendtempSound(SFXPath$+"SCP\294\Retch" + Rand(1, 2) + ".ogg", 10, 0.8)
	...
	ElseIf VomitTimer < 0 Then ;vomit
		...
		If (Not Vomit) Then
			...
			multiplayer_sendSound(VomitSFX, 10, 0.8)
		...
		If VomitTimer < -15 Then
			...
			If KillTimer >= 0 Then
				...
				multiplayer_sendSound(BreathSFX(0,0), 10, 0.8)
			EndIf
