...
Type NPCs
	...
	Field EventID%, NPCEventID%, NearestPlayer.Players
	Field waitfactor#, mpframe#
	Field x#, y#, z#, yaw#
	;{~--<END>--~}
...
Function UpdateNPCs()
	...
	For n.NPCs = Each NPCs
		...
		n\NearestPlayer = FindNearestPlayer(n)
		
		SetPlayerEventCollider(n\NearestPlayer\Pivot)
		SetPlayerEventRoom(Room[n\NearestPlayer\PlayerRoomID])
		...
		Select n\NPCtype
						...
						If n\Idle = False Then  
							...
							If dist < 15 Then
								...
								for p2.players = each players
									If (p2\BlinkTimer < - 16 Or p2\BlinkTimer > - 6) Then
										If EntityInView(n\obj, p2\PlayerBones[7]) Then
											move = False
											Exit
										EndIf
									EndIf
							...
							If move = False Then
								if n\NearestPlayer = myplayer then
							...
							Else 
								...
								Else ;less than 3 rooms away -> actively move towards the player
									...
									If temp Then 				
										If dist < 0.65 And n\NearestPlayer = myplayer Then
											If KillTimer >= 0 And (Not chs\GodMode) Then
												...
												multiplayer_SendSound(NeckSnapSFX(Rand(0,2)), 10, 0.8)
												If Rand(2) = 1 Then 
						...
						If n\State <= 0 Then	;attacking	
							...
							Else
								...
								ElseIf PlayerRoom\RoomTemplate\Name <> "gatea" And (Not chs\NoTarget) ;dist < 0.8
									...
									If Ceil(n\Frame) = 110 And (Not chs\GodMode) And n\NearestPlayer = myplayer Then
						...
						Case 2 ;being active
							...
							If (dist < HideDistance*2) And (Not n\Idle) And PlayerInReachableRoom(True) Then
								...
								If PlayerSeeAble%=True Or n\State2>0 Then ;Player is visible for 049's sight - attacking
									...
									If dist < 0.5 And n\NearestPlayer = myplayer Then
										...
										ElseIf I_1033RU\Using > 0 And I_1033RU\HP > 0 Then
									        ...
									        If BlurTimer > 500 And BlurTimer-fs\FPSfactor[0]*2.5 =< 500 Then
												...
												multiplayer_SendSound(DamageSFX1033RU(Rand(0, 3)), 10, 0.8)
												BlurTimer = 1.0
												LightFlash = 0.2
											EndIf
										Else
														...
														If (Not n\Path[n\PathLocation-1]\door\IsElevatorDoor) Then
															If EntityDistance(n\Path[n\PathLocation-1]\obj,n\Collider)>0.3 Then
																If (n\Path[n\PathLocation-1]\door\MTFClose) And (n\Path[n\PathLocation-1]\door\open) And (n\Path[n\PathLocation-1]\door\buttons[0]<>0 Or n\Path[n\PathLocation-1]\door\buttons[1]<>0) Then
																	UseDoor(n\Path[n\PathLocation-1]\door, False, True, False)
																EndIf
															EndIf
														EndIf
													EndIf
													...
													If (Not n\Path[n\PathLocation]\door\IsElevatorDoor)
														...
														Else
															If n\Path[n\PathLocation]\door\open = False And (n\Path[n\PathLocation]\door\buttons[0]<>0 Or n\Path[n\PathLocation]\door\buttons[1]<>0) Then
																UseDoor(n\Path[n\PathLocation]\door, False, True, False)
															EndIf
														EndIf
													EndIf
												EndIf
							...
							Else
								...
								If n\PathStatus = 1
									...
									Else
												...
												If n\Path[n\PathLocation-1]\door <> Null Then
													If n\Path[n\PathLocation-1]\door\KeyCard=0
														If EntityDistance(n\Path[n\PathLocation-1]\obj,n\Collider)>0.3
															If n\Path[n\PathLocation-1]\door\open Then UseDoor(n\Path[n\PathLocation-1]\door, False, True, False)
														EndIf
													EndIf
												EndIf
											EndIf
										...
										If dist2 < 0.6 Then
											If n\Path[n\PathLocation]\door <> Null Then
												If n\Path[n\PathLocation]\door\open = False Then UseDoor(n\Path[n\PathLocation]\door, False, True, False)
											EndIf
										EndIf
										...
										if n\NearestPlayer = myplayer then
											If EntityDistance(n\Collider,Collider)<1.1
												If (Abs(DeltaYaw(n\Collider,Collider))<=60.0)
													...
													multiplayer_SendSound(DamageSFX(Rand(5,8)), 10, 0.8)
													If I_1033RU\HP = 0
							...
							If SearchPlayer
								...
								If n\Reload = 0
									...
									If PickedEntity() = Collider Or n\State3=1 Then
										...
										PointEntity pvt, Collider
										Shoot(n, EntityX(pvt),EntityY(pvt), EntityZ(pvt),((25/dist_to_player)*(1/dist_to_player)), True, instaKillPlayer)
										n\Reload = 7
									Else
										n\CurrSpeed = n\Speed
									End If
										...
										If n\Reload =< 0 Then
											If dist<20.0 Then
												...
												If WrapAngle(EntityYaw(pvt)-EntityYaw(n\Collider))<10 Then
													...
													Shoot(n, EntityX(pvt),EntityY(pvt), EntityZ(pvt),((10/dist)*(1/dist))*(n\State=2),(n\State=2))
											...
											If dist < 1.8 Then
												If Abs(DeltaYaw(n\Collider, Collider))<20 Then 
													...
													Else
														...
														multiplayer_SendSound(DamageSFX(Rand(2,3)), 10, 0.8)
														If I_1033RU\HP = 0
			...
			Case NPCtype939
				...
				Select n\State
					...
					Case 2
						For e.Events = Each events
							if e\EventConst = e_room3storage Then
								n\State2 = Max(n\State2, (n\PrevState-3))
								dist = EntityDistance(n\Collider, e\room\Objects[n\State2])
								...
								PointEntity n\obj, e\room\Objects[n\State2]
								RotateEntity n\Collider, 0, CurveAngle(EntityYaw(n\obj),EntityYaw(n\Collider),20.0), 0
				...
				If n\State < 3 And (Not chs\NoTarget) And (Not n\IgnorePlayer) Then
					...
					If n\NearestPlayer\PlayerSoundVolume*1.2>dist Or dist < 1.5 Then
						If n\State3 = 0 Then
							if n\NearestPlayer = myplayer Then
								If n\Sound <> 0 Then FreeSound_Strict n\Sound : n\Sound = 0
						...
						n\State = 3
					ElseIf n\NearestPlayer\PlayerSoundVolume*1.6>dist
					...
					If (dist<HideDistance) Then
						...
						Select n\State
							...
							Case 10 ;attack
								...
								If dist<1.0 and n\NearestPlayer = myplayer Then
			...
			Case NPCtype650
				...
				If n\Idle = False Then								
                    ...
                    If PickedEntity() <> 0 Then
                        ;Do nothing
                    Else
                        If n\NearestPlayer\BlinkTimer < - 16 And EntityVisible(n\obj,n\NearestPlayer\PlayerBones[1]) Then
						    ...
						    Select Rand(5)
							    ...
							    Case 5
								    SetNPCFrame(n, 11)
						    End Select											
					    ElseIf (Not EntityInView(n\obj, n\NearestPlayer\PlayerBones[7])) And EntityVisible(n\obj,n\NearestPlayer\PlayerBones[1]) Then 
						    ...
						    Select Rand(5)
							    ...
							    Case 5
								    SetNPCFrame(n, 11)
						    End Select
					    ElseIf (Not EntityVisible(n\obj,n\NearestPlayer\PlayerBones[1])) And EntityInView(n\obj, n\NearestPlayer\PlayerBones[7]) And dist > 8.0 Then
					        ...
					        PositionEntity(n\Collider, EntityX(Collider), EntityY(Collider)+0.25, EntityZ(Collider))
					        MoveEntity(n\Collider, 0, 0, +1.5)
					    ElseIf (Not EntityVisible(n\obj,n\NearestPlayer\PlayerBones[1])) And (Not EntityInView(n\obj, n\NearestPlayer\PlayerBones[7])) And dist > 8.0 Then	
					        ;Not sure if this is buggy or not but I will let the testers test it.
		...
		Else
			n\DropSpeed = 0
		EndIf
		if not multiplayer_isahost() then
			SetAnimTime(n\obj, n\mpFrame)
			PositionEntity(n\Collider,n\x,n\y,n\z, True)
			RotateEntity(n\Collider,EntityPitch(n\Collider, True),n\yaw,EntityRoll(n\Collider, True), True)
			ResetEntity(n\Collider)
		endif
		...
		If w\door = Null Then
			...
			If xtemp < 10.0 And xtemp > 1.0 Then 
				...
				If ztemp < 10.0 And ztemp > 1.0 Then
					If (EntityDistance(n\NearestPlayer\Pivot, w\obj)>16-(8*SelectedDifficulty\aggressiveNPCs)) Then
						;teleports to the nearby waypoint that takes it closest to the player
						Local newDist# = EntityDistance(n\NearestPlayer\Pivot, w\obj)
...
Function MeNPCSeesPlayer%(me.NPCs,disablesoundoncrouch%=False)
	...
	If (Not PlayerDetected) Or (me\NPCtype <> NPCtypeMTF Or me\NPCtype <> NPCtypeMTF2)
		...
		If EntityDistance(me\NearestPlayer\Pivot,me\Collider)>(8.0-me\NearestPlayer\CrouchState+me\NearestPlayer\PlayerSoundVolume) Then Return False
		
		;spots the player if he's either in view or making a loud sound
		If me\NearestPlayer\PlayerSoundVolume>1.0
			If (Abs(DeltaYaw(me\Collider,me\NearestPlayer\Pivot))>60.0) And EntityVisible(me\Collider,me\NearestPlayer\Pivot)
				Return 1
			ElseIf (Not EntityVisible(me\Collider,me\NearestPlayer\Pivot))
			...
			EndIf
		Else
			If (Abs(DeltaYaw(me\Collider,me\NearestPlayer\Pivot))>60.0) Then Return False
		EndIf
		Return EntityVisible(me\Collider,me\NearestPlayer\Pivot)
	Else
		If EntityDistance(me\NearestPlayer\Pivot,me\Collider)>(8.0-me\NearestPlayer\CrouchState+me\NearestPlayer\PlayerSoundVolume) Then Return 3
		If EntityVisible(me\Collider, Camera) Then Return True
		
		;spots the player if he's either in view or making a loud sound
		If me\NearestPlayer\PlayerSoundVolume>1.0 Then Return 2
		Return 3
	EndIf
								...
								For r = Each Rooms
									If r\RoomTemplate\Name$ = "room173"
										...
										If Curr173\Idle = 3 And Distance(EntityX(pvt%),EntityZ(pvt%),EntityX(n\Collider),EntityZ(n\Collider)) > 4.0
											If r\RoomDoors[1]\open = True Then UseDoor(r\RoomDoors[1],False, True, False)
										EndIf
										...
										Else
											...
											r\RoomDoors[1]\MTFClose = True
                                         	UseDoor(r\RoomDoors[1],False, True, False)
						...
						If n\Reload =< 0 And KillTimer = 0 Then
							If EntityVisible(n\Collider, Camera) Then
								...
								If angle < 5 Or angle > 355 Then 
									...
									Shoot(n, EntityX(pvt),EntityY(pvt), EntityZ(pvt),((25/dist_to_player)*(1/dist_to_player)), True)
									n\Reload = 7
				...
				If n\Reload =< 0 And KillTimer = 0 Then
					If EntityVisible(n\Collider, Collider) Then
						...
						If (Abs(DeltaYaw(n\Collider,Collider))<50.0)
							...
							Shoot(n, EntityX(pvt),EntityY(pvt),EntityZ(pvt),0.9, True)
							n\Reload = 7
...
Function PlayerInReachableRoom(canSpawnIn049Chamber%=False)
	...
	temp = False
	For e = Each Events
		If e\EventConst = e_room860 And e\EventState = 1.0 Then
			temp = True
			Exit
		EndIf
	Next
	...
	If n\PathStatus <> 1
		...
		If n\CurrElevator\door\open
			If (dist# > 0.4 And dist# < 0.7) And inside%
				UseDoor(n\CurrElevator\door,False, True, False)
			EndIf
		Else
			If dist# < 0.7
				n\CurrSpeed = 0.0
...
Function ChangeNPCTextureID(n.NPCs,textureid%)
	...
	if textureid+1 < 1 Or textureid+1 > MaxDTextures Then Return
	if n\TextureID = textureid+1 Then Return
...
Function FindNearestPlayer.players(n.NPCs)
	Local nearplayer.players, dist2#
	For p.players = Each players
		if (Not p\IsDead) Then
			nearplayer = p
			dist2 = EntityDistance(n\Collider,p\Pivot)
			For ps.players = Each players
				If p <> ps And (Not ps\IsDead) Then
			...
			Next
		EndIf
		If nearplayer <> Null Then Return nearplayer
	Next
	Return myplayer
End Function
