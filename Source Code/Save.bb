Function SaveGame(file$)
	SavedPoses[0] = EntityX(Collider)
	SavedPoses[1] = EntityY(Collider)
	SavedPoses[2] = EntityZ(Collider)
	SavedPoses[3] = EntityYaw(Collider)
	SavedPoses[4] = PlayerRoom\RoomTemplate\Name
	...
	If DropSpeed#>0.02*fs\FPSfactor[0] Or DropSpeed#<-0.02*fs\FPSfactor[0] Then Return
	...
	WriteByte f, I_178\Using
	
	WriteByte f, I_215\Using
	WriteFloat f, I_215\Timer
	WriteFloat f, I_215\IdleTimer
    WriteByte f, I_215\Limit
	
	WriteByte f, I_1033RU\Using
	WriteByte f, I_1033RU\HP
	WriteByte f, I_1033RU\DHP
	...
	For i = 0 To 6
		...
		ElseIf MTF2rooms[0]<>Null
		    WriteString f, MTF2rooms[0]\RoomTemplate\Name
		Else
			WriteString f,	"a"
		EndIf
		WriteInt f, MTFroomState[i]
		WriteInt f, MTF2roomState[i]
	Next
...
Function LoadGame(file$)
	...
	MorphineHealAmount = ReadFloat(f)
	
	I_409\Timer = ReadFloat(f)
	
	I_178\Using = ReadByte(f)
	
	I_215\Using = ReadByte(f)
	I_215\Timer = ReadFloat(f)
	I_215\IdleTimer = ReadFloat(f)
    I_215\Limit = ReadByte(f)

    I_1033RU\Using = ReadByte(f)
	I_1033RU\HP = ReadByte(f)
	I_1033RU\DHP = ReadByte(f)
