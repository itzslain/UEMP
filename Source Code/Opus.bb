...
Global DEFAULT_FRAME_SIZE = 2880
		
function opus_pcm_encode(bank) ; Default frame_size is 2880
	If opusEncoder = 0 Then Return(0)
	
	local Frame_Size% = opus_get_default_frame_size()

	While BankSize(bank)/2 <> Frame_Size
		Frame_Size = Frame_Size-960
		if Frame_Size < 20 then return 0
	Wend

	local EncodeData% = CreateBank(Frame_Size)

	ResizeBank EncodeData, opus_encode(opusEncoder, bank, Frame_Size, EncodeData, banksize(EncodeData))
	
	return EncodeData
end function

function opus_pcm_decode(bank)
	If opusDecoder = 0 Then Return(0)

	Local OutputData% = CreateBank(opus_get_default_frame_size()*2)
	
	opus_decode(opusDecoder, bank, BankSize(bank), OutputData, opus_get_default_frame_size(), 0)
...
function opus_get_new_decoder()
	if opusDecoder <> 0 then opus_remove_decoder()
	opusDecoder = opus_decoder_create(opus_get_sample_rate(), opus_get_channels(), 0)
End Function

function opus_remove_decoder()
