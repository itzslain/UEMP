...
If FileSize("opus.dll")=0 Then InitErrorStr=InitErrorStr+ "opus.dll"+Chr(13)+Chr(10)
If FileSize("vlib.dll")=0 Then InitErrorStr=InitErrorStr+ "vlib.dll"+Chr(13)+Chr(10)
...
Global ModCompatibleNumber$ = "5.5.4.1"
...
Global fs.FPS_Settings = New FPS_Settings
...
fo\Font[0] = AALoadFont(FontPath$+"font\cour\Courier New.ttf", Int(19 * (GraphicHeight / 1024.0)), 0,0,0)
...
fo\Font[3] = AALoadFont(FontPath$+"DS-DIGI\DS-Digital.ttf", Int(58 * (GraphicHeight / 1024.0)), 0,0,0) ;60
		...
		If KeyHit(28) And ConsoleInput <> "" Then
			...
			Select Lower(StrTemp)
				Case "help"
					...
					Select Lower(StrTemp)
						...
						Case "playmusic"
							...
							CreateConsoleMsg("from "+Chr(34)+MusicPath+"Custom\"+Chr(34)+".")
							CreateConsoleMsg("******************************")
						Case "disable106"
						...
						Case "unlockachievements" 
							CreateConsoleMsg("HELP - unlockachievements")
							CreateConsoleMsg("******************************")
				...
				Case "scp-420-j","420j","weed"
					...
					multiplayer_SendTempSound(MusicPath +"Using420J.ogg", 10, 0.8)
					;[End Block]
				Case "godmode", "god"
					;[Block]
					StrTemp$ = Lower(Right(ConsoleInput, Len(ConsoleInput) - Instr(ConsoleInput, " ")))
				...
				Case "unlockexits"
					...
					Else
						CreateConsoleMsg("This command doesn't work for you.", 255, 0, 0)	
					EndIf
					;[End Block]
				Case "kill","suicide"
					;[Block]
...
Function UseDoor(d.Doors, showmsg%=True, playsfx%=True, give%=True)
    ;;Local fs.FPS_Settings = First FPS_Settings
	Local temp% = 0
	If d\KeyCard > 0 Then
				...
				If showmsg = True Then
					If d\locked Then
						if give = true then 
							...
							multiplayer_SendSound(KeyCardSFX2, 10, 0.8)
						EndIf
						If temp = 9 Then 
					...
					ElseIf temp = 9 Then 					
					    if give = true then 
							...
							multiplayer_SendSound(KeyCardSFX1, 10, 0.8)
						EndIf
	...
	ElseIf d\KeyCard < 0
		...
		If temp <> 0 Then
			if give = true then 
				...
				multiplayer_SendSound(ScannerSFX1, 10, 0.8)
			Endif
            ...
            If temp = 0
                If showmsg = True Then 
                   if give = true then 
						...
						multiplayer_SendSound(ScannerSFX2, 10, 0.8)
					EndIf
			...
			If showmsg = True Then 
				If Not (d\IsElevatorDoor>0) Then
					if give = true then 
						...
						multiplayer_SendSound(ButtonSFX2, 10, 0.8)
					EndIf
	...
	if give = true then multiplayer_UseDoor(d)
...
Include "Source Code\Multiplayer.bb"
...
Function MainLoop()
	...
	MouseLook((Not ms\MenuOpen) And (Not InvOpen) And (OtherOpen=Null) And (SelectedDoor = Null) And (ConsoleOpen = False) And (Using294 = False) And (SelectedScreen = Null) And I_END\Timer => 0)
	MovePlayer((Not ms\MenuOpen) And (Not InvOpen) And (OtherOpen=Null) And (SelectedDoor = Null) And (ConsoleOpen = False) And (Using294 = False) And (SelectedScreen = Null) And I_END\Timer => 0)
	...
	EndIf
	UpdateDoors()
	If QuickLoadPercent = -1 Or QuickLoadPercent = 100 Then UpdateEvents()
	UpdateScreens()
	...
	If (Not ms\MenuOpen)  Then
		...
		If KillTimer < 0 Then
			...
			If KillTimer < - 360 Then
				RevivePlayer()
				For r.Rooms = Each Rooms
					if r\Roomtemplate\Name = SavedPoses[4] Then
						PositionEntity Collider, SavedPoses[0], SavedPoses[1], SavedPoses[2]
						RotateEntity Collider, 0, SavedPoses[3], 0
						ResetEntity Collider
						PlayerRoom = r
						UpdateRooms()
						Exit
...
Function InitCredits()
	...
	fo\CreditsFont[0] = LoadFont_Strict(FontPath$+"cour\Courier New.ttf", Int(21 * (GraphicHeight / 1024.0)), 0,0,0)
	fo\CreditsFont[1] = LoadFont_Strict(FontPath$+"courbd\Courier New.ttf", Int(35 * (GraphicHeight / 1024.0)), 0,0,0)
...
Function MovePlayer(CanMove% = True)
	multiplayer_SetMyVariables()
		...
		If KeyDown(KEY_SPRINT) Then
			If Stamina < 5 Then
				...
				If ChannelPlaying(BreathCHN)=False Then 
					...
					multiplayer_SendSound(BreathSFX((temp), 0), 10, 0.8)
				EndIf
			ElseIf Stamina < 50
				If BreathCHN=0 Then
					temp = 0
					...
					multiplayer_SendSound(BreathSFX((temp), Rand(1,3)), 10, 0.8)
				Else
					If ChannelPlaying(BreathCHN)=False Then
						temp = 0
						If WearingGasMask>0 Or I_1499\Using>0 Then temp=1
	...
	If I_447\UsingEyeDrops = True Or I_447\UsingFirstAid = True Or I_447\UsingPill = True
		DeathMSG = "Site compromised by [DATA REDACTED]. Situation successfully contained after [DATA REDACTRED] hours. "
		DeathMSG = DeathMSG + "Further investigation reveals " + SubjectName$ + " in possession of an instance of SCP-447-2 through unknown means. " 
		DeathMSG = DeathMSG + SubjectName$ + " died while in possession SCP-447-2 and proceeded to cause [DATA REDACTED]."
	EndIf
	...
	if CanMove Then
		...
		If KeyHit(KEY_CROUCH) And Playable Then
			...
			multiplayer_SendSound(CrouchSFX, 10, 0.4)
			Crouch = (Not Crouch)
		EndIf
	Else
		Speed = 0
	...
	Else
		...
		local pv = CreatePivot(), ppv = CreatePivot()
		PositionEntity pv, EntityX(Collider), EntityY(Collider), EntityZ(Collider)
		PositionEntity ppv, EntityX(Collider), EntityY(Collider), EntityZ(Collider)
		
		Local ModelSpeedAngle#
		If KeyDown(KEY_DOWN) Then
			ModelSpeedAngle = 0
			If KeyDown(KEY_LEFT) Then ModelSpeedAngle = -25 
			If KeyDown(KEY_RIGHT) Then ModelSpeedAngle = 25 
		ElseIf KeyDown(KEY_UP) Then; Or ForceMove>0
			ModelSpeedAngle = 0
			If KeyDown(KEY_LEFT) Then ModelSpeedAngle = 25 
			If KeyDown(KEY_RIGHT) Then ModelSpeedAngle = -25 
		Else
			...
			If KeyDown(KEY_RIGHT) Then ModelSpeedAngle = -25
		EndIf
		ModelSpeedAngle = WrapAngle((EntityYaw(Camera))+ModelSpeedAngle+90.0)

		MoveEntity pv, -(Cos(ModelSpeedAngle)*1), 0,-(Sin(ModelSpeedAngle)*1)
		PointEntity ppv, pv
		
		myplayer\legyaw = CurveValue(EntityYaw(ppv), myplayer\legyaw, 1.5)
		...
		If CollidedFloor = True Then
			If DropSpeed# < - 0.07 Then 
				If CurrStepSFX=0 Then
					...
					multiplayer_SendSound(StepSFX(GetStepSound(Collider), 0, Rand(0, 7)), 10, 0.8)
				ElseIf CurrStepSFX=1
	...
	If Bloodloss > 0 And VomitTimer >= 0 Then
		If Rnd(200)<Min(Injuries,4.0) Then
			...
			multiplayer_SendSound(DripSFX(Rand(0, 5)), 10, 0.2)
			multiplayer_SendDecal(de)
		EndIf
	...
	If I_1079\Foam > 0 Then
		If Rnd(200)<Min(Injuries,4.0) Then
			...
			multiplayer_SendSound(SizzSFX(Rand(0,1)), 10, 0.2)
			multiplayer_SendDecal(de)
		EndIf
	...
	If ( (Not KeyDown(KEY_UP)) And (Not KeyDown(KEY_DOWN)) And (Not KeyDown(KEY_LEFT)) And (Not KeyDown(KEY_RIGHT)) ) Then
		if Crouch = 1 Then
			myplayer\PLAYER_MOVE = PLAYER_SITTING_IDLING 
		Else 
			myplayer\PLAYER_MOVE = PLAYER_IDLING
		EndIf
	Else
		if (Not KeyDown(KEY_SPRINT)) Then
			if Crouch = 1 Then
				If KeyDown(KEY_LEFT) Then 
					myplayer\PLAYER_MOVE = PLAYER_SITTING_WALKING_LEFT
				ElseIf KeyDown(KEY_RIGHT) Then 
					myplayer\PLAYER_MOVE = PLAYER_SITTING_WALKING_RIGHT
				ElseIf KeyDown(KEY_UP) Then 
					myplayer\PLAYER_MOVE = PLAYER_SITTING_WALKING_FORWARD
				ElseIf KeyDown(KEY_DOWN) Then 
					myplayer\PLAYER_MOVE = PLAYER_SITTING_WALKING_BACK
				EndIf
			Else
				If KeyDown(KEY_DOWN) Or KeyDown(KEY_UP) Or KeyDown(KEY_LEFT) Or KeyDown(KEY_RIGHT) Then myplayer\PLAYER_MOVE = PLAYER_WALKING
			EndIf
		Else
			If (Not Crouch) Then
				If KeyDown(KEY_DOWN) Or KeyDown(KEY_UP) Or KeyDown(KEY_LEFT) Or KeyDown(KEY_RIGHT) Then myplayer\PLAYER_MOVE = PLAYER_RUNNING
			EndIf
		EndIf
	EndIf
	if Not (KillTimer >= 0) then myplayer\PLAYER_MOVE = PLAYER_DEAD
	
	if (Not CanMove) Then 
		if Crouch = 1 Then myplayer\PLAYER_MOVE = PLAYER_SITTING_IDLING ELSE myplayer\PLAYER_MOVE = PLAYER_IDLING
	EndIf
	
End Function

Function MouseLook(CanMove% = True)
	Local i%, incrZoom#
	;;Local fs.FPS_Settings = First FPS_Settings
	...
	If KillTimer >= 0 And FallTimer >=0 Then
		...
		if CanMove Then
			mouse_x_speed_1# = CurveValue(MouseXSpeed() * (MouseSensitivity + 0.6) , mouse_x_speed_1, (6.0 / (MouseSensitivity + 1.0))*MouseSmooth) 
			...
			EndIf
			If InvertMouse Then
				mouse_y_speed_1# = CurveValue(-MouseYSpeed() * (MouseSensitivity + 0.6), mouse_y_speed_1, (6.0/(MouseSensitivity+1.0))*MouseSmooth) 
			Else
				mouse_y_speed_1# = CurveValue(MouseYSpeed () * (MouseSensitivity + 0.6), mouse_y_speed_1, (6.0/(MouseSensitivity+1.0))*MouseSmooth) 
			EndIf
	...
	If WearingGasmask > 0 Or I_1499\Using > 0 Then
		...
		If ChannelPlaying(BreathCHN) = False And ChannelPlaying(RelaxedBreathCHN) = False Then 
			RelaxedBreathCHN = PlaySound_Strict(RelaxedBreathSFX)
			multiplayer_SendSound(RelaxedBreathSFX, 10, 0.5)
		EndIf
					...
					If fs\FPSfactor[0]>0 Then 
						If Rand(1000)=1 Then
							If CoughCHN = 0 Then
								...
								multiplayer_SendSound(CoughSFX(Rand(0, 2)), 10, 0.8)
							Else
								If Not ChannelPlaying(CoughCHN) Then 
									CoughCHN = PlaySound_Strict(CoughSFX(Rand(0, 2)))
									multiplayer_SendSound(CoughSFX(Rand(0, 2)), 10, 0.8)
								EndIf
							End If
		...
		If MouseUp1 Then
			...
			If ClosestDoor <> Null Then 
				...
				ElseIf Playable Then
					...
					multiplayer_SendSound(ButtonSFX, 10, 0.8)
	...
	If HUDenabled Then 
		...
		If DebugHUD Then
			...
			For n2.NPCs = Each NPCs
				If n2\NPCtype = NPCtype096 Then
					AAText x - 60, 480, "SCP - 096 Position: (" + f2s(EntityX(n2\obj), 3) + ", " + f2s(EntityY(n2\obj), 3) + ", " + f2s(EntityZ(n2\obj), 3) + ")"
					AAText x - 60, 500, "SCP - 096 Idle: " + n2\Idle
					AAText x - 60, 520, "SCP - 096 State: " + n2\State
					AAText x - 60, 540, "SCP - 096 Speed: " + f2s(n2\currspeed, 5)
				EndIf
				If n2\NPCtype = NPCtype049 Then
					AAText x - 60, 560, "SCP - 049 Position: (" + f2s(EntityX(n2\obj), 3) + ", " + f2s(EntityY(n2\obj), 3) + ", " + f2s(EntityZ(n2\obj), 3) + ")"
					AAText x - 60, 580, "SCP - 049 Idle: " + n2\Idle
					AAText x - 60, 600, "SCP - 049 State: " + n2\State
					AAText x - 60, 620, "SCP - 049 Speed: " + f2s(n2\currspeed, 5)
				EndIf
				If n2\NPCtype = NPCtype650 Then
					AAText x - 60, 640, "SCP - 650 Position: (" + f2s(EntityX(n2\obj), 3) + ", " + f2s(EntityY(n2\obj), 3) + ", " + f2s(EntityZ(n2\obj), 3) + ")"
					AAText x - 60, 660, "SCP - 650 Idle: " + n2\Idle
					AAText x - 60, 680, "SCP - 650 State: " + n2\State
					AAText x - 60, 700, "SCP - 650 Speed: " + f2s(n2\currspeed, 5)
				EndIf
				If n2\NPCtype = NPCtype066 Then
					AAText x - 60, 720, "SCP - 066 Position: (" + f2s(EntityX(n2\obj), 3) + ", " + f2s(EntityY(n2\obj), 3) + ", " + f2s(EntityZ(n2\obj), 3) + ")"
					AAText x - 60, 740, "SCP - 066 Idle: " + n2\Idle
					AAText x - 60, 760, "SCP - 066 State: " + n2\State
					AAText x - 60, 780, "SCP - 066 Speed: " + f2s(n2\currspeed, 5)
				EndIf
			Next
			If PlayerRoom\RoomTemplate\Name$ = "dimension1499"
		...
		If SelectedItem <> Null Then
			If SelectedItem\itemtemplate\tempname = "scp005" Then 
				...
				If SelectedDoor\Code<>"ABCD" Then
					...
					SelectedDoor = Null
					PlaySound_Strict ScannerSFX1
					multiplayer_SendSound(ScannerSFX1, 10, 0.7)
					Msg = "You hold the key close to the keypad."
					MsgTimer = 70 * 5
				Else
					SelectedDoor = Null
					PlaySound_Strict ScannerSFX2
					multiplayer_SendSound(ScannerSFX2, 10, 0.7)
						...
						If MouseHit1 Then
							...
							If DoubleClick Then
								If WearingHazmat > 0 And Instr(SelectedItem\itemtemplate\tempname,"hazmatsuit")=0 Then
									...
									Return
								EndIf
								If Inventory(n)\itemtemplate\sound <> 66 Then 
									PlaySound_Strict(PickSFX(Inventory(n)\itemtemplate\sound))
									multiplayer_SendSound(PickSFX(Inventory(n)\itemtemplate\sound), 10, 0.7)
								EndIf
								InvOpen = False
								DoubleClick = False
							EndIf
		...
		If SelectedItem <> Null Then
			Select SelectedItem\itemtemplate\tempname
				Case "nvgoggles"
					...
					If I_215\Using > 0
						Msg = "You need to take off the SCP-215 in order to put on the goggles."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
					...
					ElseIf I_1499\Using > 0
                        Msg = "You need to take off the SCP-1499 in order to put on the goggles."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
					ElseIf I_178\Using > 0
                        Msg = "You need to take off the SCP-178 in order to put on the goggles."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
				...
				Case "scp513"
					...
					multiplayer_SendTempSound(SFXPath$+"SCP\513\Bell1.ogg")
				...
				Case "scp500pill"
					;[Block]
					If I_1499\Using > 0
					    Msg = "You can't swallow the pill while wearing the SCP-1499."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf WearingGasMask > 0
					    Msg = "You can't swallow the pill while wearing a gas mask."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    Else
				...
				Case "veryfinefirstaid"
					;[Block]
					If I_1499\Using > 0
					    Msg = "You can't use that item wearing the SCP-1499."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf WearingGasMask > 0
					...
					ElseIf I_215\Using > 0
					    Msg = "You can't use that item while wearing the SCP-215."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    ElseIf I_178\Using > 0
					    Msg = "You can't use that item while wearing the SCP-178."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    ElseIf WearingNightVision > 0
					    Msg = "You can't use that item while wearing a goggles."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    Else
				...
				Case "eyedrops", "eyedrops2"
					;[Block]
					If I_1499\Using > 0
					    Msg = "You can't use the eyedrops wearing the SCP-1499."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf WearingGasMask > 0
					    Msg = "You can't use the eyedrops while wearing a gas mask."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf I_215\Using > 0
					    Msg = "You can't use the eyedrops while wearing the SCP-215."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf I_178\Using > 0
					    Msg = "You can't use the eyedrops while wearing the SCP-178."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf WearingNightVision > 0
					    Msg = "You can't use the eyedrops while wearing a goggles."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    Else
					...
					If SelectedItem\itemtemplate\img = 0 Then
						Select SelectedItem\itemtemplate\name
							...
							Case "Document SCP-XXX"
								...
								SelectedItem\itemtemplate\img = ResizeImage2(SelectedItem\itemtemplate\img, ImageWidth(SelectedItem\itemtemplate\img) * MenuScale, ImageHeight(SelectedItem\itemtemplate\img) * MenuScale)
								If (SelectedItem\state = 0) Then
									Msg = Chr(34)+"How strange, this document is different from the others. I think..."+Chr(34)
									MsgTimer = 70*10
									PlaySound_Strict LoadTempSound(SFXPath$+"SCP\1162\NostalgiaCancer"+Rand(1,5)+".ogg")
									SelectedItem\state = 1
								EndIf
				...
				Case "cup"
					;[Block]
					If I_1499\Using > 0
					    Msg = "You can't use that item while wearing the SCP-1499."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf WearingGasMask > 0
				...
				Case "cigarette"
					;[Block]
					If I_1499\Using > 0
					    Msg = "You can't use the cigarette while wearing the SCP-1499."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf WearingGasMask > 0
					    Msg = "You can't use the cigarette while wearing a gas mask."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    Else
				...
				Case "scp420j"
					;[Block]
					If I_1499\Using > 0
					    Msg = "You can't use the SCP-420-J while wearing the SCP-1499."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf WearingGasMask > 0
					    Msg = "You can't use the SCP-420-J while wearing a gas mask."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    Else
						...
						Else
							...
							PlaySound_Strict LoadTempSound(MusicPath + "Using420J.ogg")
							multiplayer_SendTempSound(MusicPath + "Using420J.ogg", 10, 0.4)
				...
				Case "gasmask", "supergasmask", "gasmask3"
                    ...
                    For i = 0 To MaxItemAmount - 1
                        If Inventory(i) <> Null Then
                            If Inventory(i)\itemtemplate\name = "SCP-215" And I_215\Timer >= 86.0 Then
                                Msg = "You can't wear a gas mask because of the glasses."
						        MsgTimer = 70 * 5
						        SelectedItem = Null
						        Return
                                Exit
					...
					If I_1499\Using > 0
					    Msg = "You need to take off the SCP-1499 in order to put on a gas mask."	
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf I_178\Using > 0 Then
					    Msg = "You need to take off the SCP-178 in order to put on a gas mask."
                        MsgTimer = 70 * 5
                        SelectedItem = Null
					    Return
					ElseIf I_215\Using > 0 Then
					    Msg = "You need to take off the SCP-215 in order to put on a gas mask."
                        MsgTimer = 70 * 5
                        SelectedItem = Null
					    Return
					ElseIf WearingNightVision > 0
					    ...
					    Return
                    ElseIf WearingHelmet > 0
                        Msg = "You need to take off the helmet in order to put on a gas mask."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
                    Else		
				...
				Case "scp1499", "super1499"
					...
					For i = 0 To MaxItemAmount - 1
                        If Inventory(i) <> Null Then
                            If Inventory(i)\itemtemplate\name = "SCP-215" And I_215\Timer >= 86.0 Then
                                Msg = "You can't wear the SCP-1499 because of the SCP-215."
						        MsgTimer = 70 * 5
						        SelectedItem = Null
						        Return
                                Exit
					...
					If WearingNightVision > 0
						Msg = "You need to take off the goggles in order to put on the SCP-1499."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
				    ElseIf WearingGasMask > 0
				        Msg = "You need to take off the gas mask in order to put on the SCP-1499."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
					ElseIf I_178\Using > 0
                        Msg = "You need to take off the SCP-178 in order to put on the SCP-1499."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
					ElseIf I_215\Using > 0
                        Msg = "You need to take off the SCP-215 in order to put on the SCP-1499."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
					ElseIf WearingHelmet > 0
                        Msg = "You need to take off the helmet in order to put the SCP-1499."
						MsgTimer = 70 * 5
						SelectedItem = Null
					    Return
                    Else				
                ...
                Case "minteyedrops", "minteyedrops2"
					;[Block]
					If I_1499\Using > 0
					    Msg = "You can't use the minty eyedrops wearing the SCP-1499."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf WearingGasMask > 0
					    Msg = "You can't use the minty eyedrops while wearing a gas mask."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf I_215\Using > 0
					    Msg = "You can't use the minty eyedrops while wearing the SCP-215."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    ElseIf I_178\Using > 0
					    Msg = "You can't use the minty eyedrops while wearing the SCP-178."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    ElseIf WearingNightVision > 0
					    Msg = "You can't use the minty eyedrops while wearing a goggles."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    Else
					    If (Not I_402\Using)
					        Msg = "The eyedrops smell of mint. Your eyes feel very moisturized."
					        MsgTimer = 70*5
						    BlinkEffect = 0.5
						    BlinkEffectTimer = Rand(30,40)
						    BlurTimer = 200
				...
				Case "morphine"
				 ...
				 Case "mintscp500pill"
					;[Block]
					If I_1499\Using > 0
					    Msg = "You can't swallow the minty pill while wearing the SCP-1499."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf WearingGasMask > 0
					    Msg = "You can't swallow the minty pill while wearing a gas mask."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    Else
                ...
                Case "scp178"
				    ...
				    ElseIf WearingGasMask > 0
						...
						Return
					ElseIf I_1499\Using > 0
                        Msg = "You need to take off the SCP-1499 in order to put on the SCP-178."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
					ElseIf I_215\Using > 0
                        Msg = "You need to take off the SCP-215 in order to put on the SCP-178."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
					ElseIf WearingHelmet > 0
                        Msg = "You need to take off the helmet in order to put on the SCP-178."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
                    Else			
				        If I_178\Using = 1 Then
						    Msg = "You removed the SCP-178."
						    I_178\Using = 0
					    Else
						    GiveAchievement(Achv178)
						    Msg = "You put on the SCP-178."
						    I_178\Using = 1
					    EndIf
                        MsgTimer = 70 * 5
			            SelectedItem = Null
                ...
                Case "scp215"
				    ...
				    ElseIf WearingGasMask > 0
						...
						Return
					ElseIf I_1499\Using > 0
                        Msg = "You need to take off the SCP-1499 in order to put on the SCP-215."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
					ElseIf I_178\Using > 0
                        Msg = "You need to take off the SCP-178 in order to put on the SCP-215."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
					ElseIf WearingHelmet > 0
                        Msg = "You need to take off the helmet in order to put on the SCP-215."
						MsgTimer = 70 * 5
						SelectedItem = Null
						Return
                    Else			
                        If I_215\Using = 1 Then
						    Msg = "You removed the SCP-215."
						    I_215\Using = 0
					    Else
						    GiveAchievement(Achv215)
						    Msg = "You put on the SCP-215."
						    I_215\Using = 1					
					    EndIf
                        MsgTimer = 70 * 5
			            SelectedItem = Null
						...
						For n% = 0 To ItemAmount + 0
						    If Inventory(n) = Null Then
						        ...
						        Else
									...
									Inventory(n)\Picker = myplayer\ID
			    ...
			    Case "scp1079sweet"
                    ;[Block]
				     If I_1499\Using > 0
					    Msg = "You can't swallow the candy while wearing the SCP-1499."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf WearingGasMask > 0
					    Msg = "You can't swallow the candy while wearing a gas mask."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    Else
				        Select Rand(2)
				            ...
				            Case 2
						        Msg = "You swallowed candy. You feel a good taste."
						End Select
						MsgTimer = 70*7
				...
				Case "scp207"
				    ;[Block]
				    If I_1499\Using > 0
					    Msg = "You can't drink the cola while wearing the SCP-1499."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf WearingGasMask > 0
					    Msg = "You can't drink the cola while wearing a gas mask."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    Else
				        If (Not I_402\Using)
							...
							If SelectedItem\state = 100 Then
							    If SelectedItem\itemtemplate\tempname = "scp207" Then
									...
									multiplayer_SendTempSound(SFXPath$+"SCP\109\ahh.ogg", 10, 0.8)
				...
				Case "scp109"
				    ;[Block]
				    If I_1499\Using > 0
					    Msg = "You can't drink the water while wearing the SCP-1499."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf WearingGasMask > 0
					    Msg = "You can't drink the water while wearing a gas mask."
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
                    Else
				...
				Case "scp402"
				    ...
				    If WearingGasmask = 0 And I_1499\Using = 0 Then
				        If I_402\Using = 1 Then
						    Msg = "You pull out the SCP-402 from your mouth."
						    I_402\Using = 0
					    Else
						    GiveAchievement(Achv402)
						    Msg = "You put the SCP-402 in your mouth."
						    I_402\Using = 1
					    EndIf
				...
				Case "helmet"
					...
					For i = 0 To MaxItemAmount - 1
                        If Inventory(i) <> Null Then
                            If Inventory(i)\itemtemplate\name = "SCP-215" And I_215\Timer >= 86.0 Then
                                Msg = "You can't wear the helmet because of the glasses."
						        MsgTimer = 70 * 5
						        SelectedItem = Null
						        Return
                                Exit
					...
					If I_1499\Using > 0
					    Msg = "You need to take off the SCP-1499 in order to put on a helmet."	
					    MsgTimer = 70 * 5
					    SelectedItem = Null
					    Return
					ElseIf WearingNightVision > 0
				    ...
				    ElseIf WearingGasMask > 0
					    ...
					    Return
					ElseIf I_178\Using > 0
					    Msg = "You need to take off the SCP-178 in order to put on a helmet."
                        MsgTimer = 70 * 5
                        SelectedItem = Null
					    Return
                    ElseIf I_215\Using > 0
					    Msg = "You need to take off the SCP-215 in order to put on a helmet."
                        MsgTimer = 70 * 5
                        SelectedItem = Null
					    Return
                    Else		
				...
				Case "mintpill"
					...
					ElseIf WearingGasMask > 0
					    ...
					    Return
                    Else
						Msg = "You swallowed the minty pill."
						MsgTimer = 70*7
...
Function DrawMenu()
	...
	If ms\MenuOpen Then
		...
		ElseIf AchievementsMenu <= 0 And OptionsMenu > 0 And QuitMSG <= 0 And KillTimer >= 0
			...
			Select OptionsMenu
				...
				Case 3 ;Controls
					...
					Color 0, 200, 0
					AAText(x, y + 240 * MenuScale, "Voice")
					Color 255,255,255
					InputBox(x + 200 * MenuScale, y + 240 * MenuScale,100*MenuScale,20*MenuScale,KeyName(Min(KEY_VOICE,210)),14)
				...
				Case 4 ;Advanced
					...
					If CanOpenConsole
					    ...
					    Else
					        AAText(x + 310 * MenuScale, y, "Old Version")
					    EndIf    
					    If MouseOn(x+270*MenuScale,y+MenuScale,20*MenuScale,20*MenuScale)
		...
		If AchievementsMenu<=0 And OptionsMenu<=0 And QuitMSG<=0 Then
			...
			Else
				y = y+107*MenuScale
				If DrawButton(x, y, 430*MenuScale, 60*MenuScale, "Respawn") Then
					RevivePlayer()
					For r.Rooms = Each Rooms
						if r\Roomtemplate\Name = SavedPoses[4] Then
							PositionEntity Collider, SavedPoses[0], SavedPoses[1], SavedPoses[2]
...
Function LoadEntities()
	...
	o\NPCModelID[0] = LoadAnimMesh_Strict(NPCsPath$+"scp_173.b3d") ;SCP-173
	AddAnimSeq(o\NPCModelID[0], 1)
	SetAnimTime(o\NPCModelID[0], 0) ; Starting anim mesh animations
	...
	o\NPCModelID[33] = LoadAnimMesh_Strict(NPCsPath$+"scp_682_arm.b3d") ;SCP-682's Arm
	
	o\NPCModelID[34] = LoadAnimMesh_Strict(NPCsPath$+"scp_173_box.b3d") ;SCP-173's Box
...
Function Use914(item.Items, setting$, x#, y#, z#)
	if not multiplayer_IsAHost() Then Return
	...
	Select item\itemtemplate\name
		...
		Default
			...
			;[End Block]
	End Select
	if d <> Null Then multiplayer_SendDecal(d)
			...
			If temp And Input294<>"" Then ;dispense
				...
				If loc > 0 Then
					...
					If strtemp="" Then
						...
						multiplayer_SendTempSound(SFXPath$+"SCP\294\Dispense1.ogg")
					Else
						PlayerRoom\SoundCHN = PlaySound_Strict (LoadTempSound(strtemp))
						multiplayer_SendTempSound(strtemp, 10, 1)
					EndIf
				...
				Else
					...
					multiplayer_SendTempSound(SFXPath$+"SCP\294\OutOfRange.ogg", 10, 1)
				EndIf
...
Function Use215() ;The first UE's object
    ...
    If I_215\Timer > 0 Then
        ...
        If I_215\Timer > 30.0 And prevI215Timer =< 30.0 Then
            BlurTimer = 3000
            Msg = "The walls seems alive for you."
			MsgTimer = 70*6
        EndIf
	    ...
	    If I_215\Timer > 60.0 And prevI215Timer =< 60.0 Then
	        Msg = Chr(34)+"It seems to me that everything around me is alive."+Chr(34)
	        MsgTimer = 70*6
	    EndIf
	    If I_215\Timer > 65.0 And prevI215Timer =< 65.0 Then
	        Msg = Chr(34)+"How are you, Paul?"+Chr(34)
	        MsgTimer = 70*6
	    EndIf
		If I_215\Timer > 70.0 And prevI215Timer =< 70.0 Then
		    BlurTimer = 5200
		    CameraShake = 22
			Msg = "You have a paranoia."
			MsgTimer = 70*6
			PlaySound_Strict I_215\Sound[0]
		EndIf
	    If I_215\Timer > 75.0 And prevI215Timer =< 75.0 Then
	        CameraShake = 30
	        Msg = Chr(34)+"Why are you not talking with me?"+Chr(34)
	        MsgTimer = 70*6
	    EndIf
		If I_215\Timer > 86.0 And prevI215Timer =< 86.0 Then
		    ...
		    For i=0 To MaxItemAmount-1
                If Inventory(i)<>Null Then
                    If Inventory(i)\itemtemplate\name="SCP-215" Then
                        Msg = "You don't want to take your glasses off."
			            MsgTimer = 70*6
...
Function Use207()
      ...
      If I_207\Timer > 0.0 Then
          ...
          If I_207\Timer >= 50.0 Then
              DeathMSG = SubjectName$+". Cause of death: organ failure due to massive internal hemorrhage. As SCP-207-A was empty on recovery, it's believed ingesting a full bottle [DATA REDACTED]."
              Kill()
          EndIf
      EndIf
End Function
    ...
    If I_402\Using > 0 Then
        If I_402\Timer >= 0 Then
            ...
            If I_402\Timer > 10.0 And prevI402Timer =< 10.0 Then
                ...
                CameraShake = 5
                If I_1033RU\HP = 0
                    Msg = "SCP-402 hit you in the throat."
                    Injuries = Injuries + 0.1
                Else
                    Msg = "SCP-402 hit you in the throat, but SCP-1033-RU protected you."
            ...
            If I_1033RU\HP > 0
                If I_402\Timer > 12.0 Then
                    Msg = "SCP-1033-RU pulled out the SCP-402 from your throat."
                    MsgTimer = 70*6
                    Damage1033RU(10 + (Rand(5) * SelectedDifficulty\aggressiveNPCs))
                    PlaySound_Strict(CoughSFX(Rand(0, 2)))
					multiplayer_SendSound(CoughSFX(Rand(0, 2)), 10, 0.8)
            ...
            If I_402\Timer > 40.0 And prevI402Timer =< 40.0 Then
                Msg = "You breathe heavily. You can't pull out the stone from your throat."
			    MsgTimer = 70*6
                PlaySound_Strict(CoughSFX(Rand(0, 2)))
				multiplayer_SendSound(CoughSFX(Rand(0, 2)), 10, 0.8)
                CameraShake = 5
	        ...
	        If I_402\Timer >= 60.0 And prevI402Timer =< 60.0 Then
	            PlaySound_Strict(CoughSFX(Rand(0, 2)))
				multiplayer_SendSound(CoughSFX(Rand(0, 2)), 10, 0.8)
                DeathMSG = "The emaciated body of "+SubjectName$+" is found in [DATA REDACTED]. SCP-402 was discovered in the subject's lungs, death was caused by suffocation." ;REALLY??? SUFFOCATION??? I will remove this SCP in future version - Jabka
		        Kill()
	        EndIf   
	    EndIf
	Else
...
Function Use357() 
     ...
     If I_357\Timer > 0 Then
		 ...
		 If I_357\Timer > 40.0 And prevI357Timer =< 40.0 Then
		     BlurTimer = 5000
			 Msg = "Your fingers on your hand are immersed inside the SCP-357."
			 MsgTimer = 70*6
		 EndIf
         If I_357\Timer > 56.0 And prevI357Timer =< 56.0 Then
             BlurTimer = 5000
             Msg = "You can't move your fingers."
             MsgTimer = 70*6
         EndIf
	     ...
	     If I_357\Timer >= 75.0 Then
	         If Rand(2) = 1 Then
	             DeathMSG = SubjectName$+" found dead in Sector [DATA REDACTED] without right hand. "
	         Else
	             DeathMSG = SubjectName$+" found dead in Sector [DATA REDACTED] without left hand. "
             EndIf
	         DeathMSG = DeathMSG + "Probably subject had physical contact with SCP-357."
	         Kill()
	     EndIf
     EndIf
...
Function UpdateMTF()
	...
	;mtf ei viel?¤ spawnannut, spawnataan jos pelaaja menee tarpeeksi l?¤helle gate a:t?¤
	If MTFtimer = 0 Then
		If Rand(30) = 1 Then
			...
			If entrance <> Null Then
				For p.players = Each players
					If Abs(EntityZ(entrance\obj)-EntityZ(Collider))<30.0 Then
						if Room[p\PlayerRoomID]\RoomTemplate\Name$ <> "dimension1499" And Room[p\PlayerRoomID]\RoomTemplate\Name <> "gateaentrance" Then
						
							If PlayerInReachableRoom()
								PlayAnnouncement(SFXPath$+"Character\MTF\Announc.ogg", True)
	...
	Else
        ...
        ElseIf MTFtimer > 70*90 And MTFtimer < 70*91
            If PlayerInReachableRoom()
                PlayAnnouncement(SFXPath$+"Character\MTF2\Announc.ogg", True)
            EndIf
            MTFtimer = 70*91
		...
		ElseIf MTFtimer > 70*131 And MTFtimer < 10000
			If PlayerInReachableRoom()
				PlayAnnouncement(SFXPath$+"Character\MTF\AnnouncAfter1.ogg", True)
			EndIf
			MTFtimer = 10000
		...
		ElseIf MTFtimer > 10000+(70*120) And MTFtimer < 20000
			If PlayerInReachableRoom()
				PlayAnnouncement(SFXPath$+"Character\MTF\AnnouncAfter2.ogg", True)
			EndIf
			MTFtimer = 20000
		...
		ElseIf MTFtimer > 20000+(70*60) And MTFtimer < 25000
			If PlayerInReachableRoom()
				...
				For it.Items = Each items
					if it\Picker <> 0 Then
						If (Left(it\itemtemplate\name, 4) = "SCP-") And (Left(it\itemtemplate\name, 7) <> "SCP-035") And (Left(it\itemtemplate\name, 7) <> "SCP-093") And (Left(it\itemtemplate\name, 7) <> "SCP-085") And (Left(it\itemtemplate\name, 7) <> "SCP-457") And (Left(it\itemtemplate\name, 7) <> "SCP-3651")
							PlayAnnouncement(SFXPath$+"Character\MTF\ThreatAnnouncPossession.ogg", True)
							MTFtimer = 25000
							Return
							Exit
						EndIf
				...
				PlayAnnouncement(SFXPath$+"Character\MTF\ThreatAnnounc"+Rand(1,3)+".ogg", True)
			EndIf
			MTFtimer = 25000
		...
		ElseIf MTFtimer > 25000+(70*60) And MTFtimer < 30000
			If PlayerInReachableRoom()
				PlayAnnouncement(SFXPath$+"Character\MTF\ThreatAnnouncFinal.ogg", True)
			EndIf
			MTFtimer = 30000
			...
			If entrance2 <> Null Then
				For p.players = Each players
					If Abs(EntityZ(entrance2\obj)-EntityZ(Collider))<30.0 Then
						if Room[p\PlayerRoomID]\RoomTemplate\Name <> "gateb" And Room[p\PlayerRoomID]\RoomTemplate\Name$ <> "dimension1499"
							MTF2timer = 1.0
			...
			If teleportForInfect
				...
				ElseIf I_008\Timer < 98.5
					...
					If prevI008Timer < 94.7 Then 
						...
						DeathMSG = SubjectName$+" found ingesting Dr. [DATA REDACTED] at Sector [DATA REDACTED]. Subject was immediately terminated by Nine-Tailed Fox and sent for autopsy. "
...
Function Graphics3DExt%(width%,height%,depth%=32,mode%=2)
	...
	InitFastResize()
	;InitExt()
	if not api_IsWindowVisible(SystemProperty("AppHWND")) then api_showWindow(SystemProperty("AppHWND"), 1)
...
Function CheckTriggers$()
	...
	If PlayerRoom\TriggerboxAmount = 0
		if getpreviouseventcollider() <> collider and getpreviouseventcollider() <> 0 then debuglog "Shit!"
		Return ""
	Else
		For i = 0 To PlayerRoom\TriggerboxAmount-1
			...
			If EntityX(Collider, True) > ((sx#*Mesh_MinX)+PlayerRoom\x) And EntityX(Collider, True) < ((sx#*Mesh_MaxX)+PlayerRoom\x)
				If EntityY(Collider, True) > ((sy#*Mesh_MinY)+PlayerRoom\y) And EntityY(Collider, True) < ((sy#*Mesh_MaxY)+PlayerRoom\y)
					If EntityZ(Collider, True) > ((sz#*Mesh_MinZ)+PlayerRoom\z) And EntityZ(Collider, True) < ((sz#*Mesh_MaxZ)+PlayerRoom\z)
						inside% = i%
						Exit
					EndIf
				EndIf
...
Function ToValue#(number#, old#, speed#)
	...
	If number < old Then
		Return Max(number, old-(speed*fs\FPSfactor[0]))
	Else
		Return Min(number, old+(speed*fs\FPSfactor[0]))
	EndIf
End Function
...
Function Update3DSounds()
	For snd.snd3d = Each snd3d
		...
		if Not ChannelPlaying(snd\soundchn) Then 
			...
			if snd\sound <> 0 Then FreeSound(snd\sound)
