...
Function UpdateSoundOrigin2(Chn%, cam%, entity%, range# = 10, volume# = 1.0)
	...
	If volume>0 Then
		...
		If 1 - dist# > 0 And 1 - dist# < 1 Then
			...
			ChannelVolume(Chn, Min(1.0, volume# * (1 - dist#)))
			ChannelPan(Chn, panvalue)
		EndIf
	Else
		If Chn <> 0 Then
...
Function UpdateSoundOrigin(Chn%, cam%, entity%, range# = 10, volume# = 1.0)
	...
	If volume>0 Then
		...
		If 1 - dist# > 0 And 1 - dist# < 1 Then
			...
			ChannelVolume(Chn, Min(1.0, volume# * (1 - dist#)*SFXVolume#))
			ChannelPan(Chn, panvalue)
		EndIf
	Else
		If Chn <> 0 Then
...
Function PlayAnnouncement(file$, Synchronize% = False) ;This function streams the announcement currently playing
	...
	if Synchronize Then multiplayer_SendAnnouncement(file)
End Function
